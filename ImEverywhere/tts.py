...
# Pre initialized to accelerate the 'ttsBaidu'
pygame.mixer.init()
key = "QrhsINLcc3Io6w048Ia8kcjS"
secret_key = "e414b3ccb7d51fef12f297ffea9ec41d"
access_token = getToken(key, secret_key)
mac_address = uuid.UUID(int=uuid.getnode()).hex[-12:]	


def ttsBaidu(info, language='zh'):     
...
def ttsIflytek(info, language='zh'):
	pass
...
if __name__ == "__main__":
    ...
    # Automatic language detection, the default ttsBaidu mode is 'zh' represents Chinese
	while True:
		try:
			question = input(">>")
			inputLang = langid.classify(question)[0]
			ttsBaidu(question, language=inputLang)
